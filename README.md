...
在已加载本次路由更新的主控中，可在任务编辑页把“默认运行电脑（手动和计划）”设为 B，然后从任务中心点“运行”；页面会打开 B 的远程运行记录。B 离线时任务留在 B 排队，不会改投 A。历史未绑定任务仍在 A 本机运行。管理员也可在“宿主机”页直接向 B 分发；当前正式进程是否已加载更新，以发布和重启记录为准。
