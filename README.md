...
电脑 B 同样需要 Windows 10/11 和 64 位 Python 3.12。

1. 解压 Agent ZIP。
2. 双击 `安装并接入Agent.bat`。
3. 填写主控地址，例如 `http://192.168.1.50:8000`。
4. 填写一次性接入码和容易辨认的电脑名称。
5. 等待电脑 A 的管理员在“宿主机”页面批准。

向导会检查 Python 和主控连接，并可设置“登录 Windows 后自动启动 Agent”。批准后，Agent 状态会显示在 Windows 托盘和主控页面中。
