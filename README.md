<p align="center">
  <img src="assets/readme/hero.svg" width="100%" alt="SpiderFly：把 Python 任务放到一台共享 Windows，团队通过浏览器提交，排队运行并下载结果。">
</p>

# SpiderFly

团队共用一台 Windows 电脑运行 Python 自动化。成员在浏览器里提交任务、安排计划、查看日志和下载结果。**同一时间执行一个任务，其余任务进入持久队列。**

[快速开始](#快速开始) · [运行任务](#运行-python-任务) · [项目架构](docs/项目架构.md) · [使用说明](使用说明.md) · [全部文档](docs/README.md)

## 团队日常工作

| 要做的事 | SpiderFly 提供什么 |
| --- | --- |
| 共用脚本 | 浏览器入口、独立账号、固定任务归属和操作记录 |
| 定时执行 | 手动、每日、每周触发，共用持久队列 |
| 找到结果 | 运行历史、标准输出、错误日志和本次文件下载 |
| 管理依赖与代码 | 任务独立 Python 环境、代码版本和回退 |
| 准备新任务 | AI 生成草稿、探索公开网页并提取数据 |
| 排查故障 | AI 在支持的受限环境验证修复，通过后启用并自动重跑一次 |

正常运行已保存脚本不调用模型。AI 创建、对话和故障维护使用配置的 DeepSeek 服务；自动修复仍受环境、超时和后台用量保护约束。

## 平台管调度，脚本管业务

<p align="center">
  <img src="assets/readme/architecture.svg" width="100%" alt="平台负责账号、调度、环境和记录；普通 Python 脚本负责业务；失败进入 AI 验证与自动重跑。">
</p>

| 层 | 位置 | 责任 |
| --- | --- | --- |
| 平台 | `backend/app` · `frontend/src` | 接收任务、准备环境、串行执行、保存记录 |
| 业务脚本 | `examples` · 用户上传的 `.py` | 具体字段、条件、计算与采集逻辑 |

业务直接写 Python，按需使用标准库、openpyxl 等依赖。指令框架、兼容包及衍生运行库已全部移除。详见 [项目架构](docs/项目架构.md)。

## 快速开始

**宿主机：Windows 10/11 + 64 位 Python 3.12。** 首次安装需要访问 Python 包源；成员电脑只需要浏览器。

```powershell
git clone https://github.com/flyyang12-rgb/spiderFly.git
cd spiderFly
.\start.bat
```

`start.bat` 准备平台环境并启动网页。默认本机地址为 `http://127.0.0.1:8000`，首次登录信息位于 `data/首次登录信息.txt`；之后日常双击 `SpiderFly.exe`。

仓库包含构建好的前端，普通部署无需 Node.js。团队访问与防火墙配置见 [部署指南](docs/部署与操作详解.md#局域网访问)。

## 运行 Python 任务

1. 准备一个 `.py` 入口，按脚本实际需要填写第三方依赖。
2. 管理员上传脚本，填写任务名称和说明，选择手动或定时触发。
3. 等待“Python 就绪”后运行，在运行中心查看排队状态和日志。
4. 脚本如有输出文件，将其保存到本次产物目录，再从运行记录下载。

业务逻辑直接写在 Python 中。仅使用标准库的脚本无需填写第三方依赖；平台提供调度、环境、日志和结果管理。详细步骤见 [使用说明](使用说明.md)。

## AI 遇错自动修复

失败或超时 → AI 生成最小修复 → 受限环境验证 → 自动启用 → 排队重跑一次。原错误和原版本保留，验证失败不替换代码，重跑失败不循环。需配置 AI 服务和支持的验证环境，详见 [自动修复说明](docs/自动修复.md)。

## 使用边界

- **一台宿主机，串行执行。** 当前不提供多机 Agent 调度或多个桌面任务并行。
- **桌面软件独立准备。** Office、浏览器、驱动和业务登录不由 Python 依赖清单安装。
- **自动维护有环境范围。** 支持已准备的 WSL readonly-v1 / collection-v1；不承诺任意 Windows GUI、登录软件或未知依赖都能自动修复。
- **探索会话与定时脚本分开。** 浏览器登录状态不会自动转入定时脚本。
- **运行资料留在宿主机。** 真实配置、任务、业务文件和账号不进入仓库；升级前按 [备份与恢复](使用说明.md#十二备份和换电脑恢复) 操作。

## 文档入口

[使用说明](使用说明.md) · [部署详解](docs/部署与操作详解.md) · [AI 任务助手](docs/AI任务助手.md) · [AI 自动修复](docs/自动修复.md) · [开发与验证](docs/开发与验证.md) · [维护约定](AGENTS.md)

完整索引见 [docs](docs/README.md)。当前状态与本机待发布改动见 [CONTEXT](CONTEXT.md)。

仓库暂未附带开源许可证；公开可见不代表已经授予任意复制、修改或再发布的许可。
