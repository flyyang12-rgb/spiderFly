      ...
      <div class="ai-layout">
        ...
        <aside class="ai-side">
          ...
          <article v-for="draft in visibleDrafts" :key="draft.id" class="ai-draft">
            <strong>V{{ draft.version }} · {{ draft.name }}</strong><div class="ai-draft-status"><span class="mini-badge neutral-badge">语法通过 · 未试运行</span></div>
            <details v-if="draft.description" class="ai-disclosure"><summary>草稿说明</summary><p>{{ draft.description }}</p></details>
            <div class="ai-draft-actions"><button class="button ghost" @click="preview(draft)">查看代码</button><button class="button primary" :disabled="Boolean(active)" @click="useDraft(draft)">{{ thread?.task_id ? '创建测试任务' : '使用草稿' }}</button></div>
