...
`runner.py` 执行任务，`main.py` 串行 worker 触发维护，`maintenance.py` 负责生成、验证、版本发布和重跑，`maintenance_runtime.py` 负责受限环境。人工修改版本由 `task_versions.py` 管理。

## 维护边界

`main.py` 和 `App.vue` 仍有职责集中，后续拆分应单独验证。正式数据和任务版本只能通过平台流程维护；修改仓库示例不会替换已上传任务。详细约束见 [AGENTS](../AGENTS.md) 和 [任务执行约定](任务执行约定.md)。
