...
import sqlite3
import unittest
from app import database, runner, scheduling, security
from app.api import executions as executions_api, tasks as tasks_api
...
class ScheduledQueueTests(unittest.TestCase):
    ...
    def tick(self, now: str) -> None:
        with (
            ...
            with self.assertRaises(asyncio.CancelledError):
                asyncio.run(scheduling.scheduler_loop(execution_queue._enqueue_task))
    ...
    def test_due_schedule_runs_even_when_manual_execution_is_pending_or_running(self):
        for state in ("pending", "running"):
            with self.subTest(state=state), self.helper.fixture() as item:
                ...
                manual_id = execution_queue._enqueue_task_sync(task["id"], "manual", item["user"]["id"])
                self.assertEqual(database.fetch_one("SELECT next_run_at FROM tasks WHERE id = ?", (task["id"],))["next_run_at"], "2026-09-07T01:00:00+00:00")
                if state == "running":
                    self.assertEqual(execution_queue._claim_next_execution(), manual_id)
    ...
    def test_manual_duplicate_protection_stays_in_place(self):
        for source in ("manual", "schedule"):
            with self.subTest(source=source), self.helper.fixture() as item:
                task = self.helper.create_task(item["app_id"], item["user"])
                execution_queue._enqueue_task_sync(task["id"], source)
                with self.assertRaises(HTTPException) as caught:
                    execution_queue._enqueue_task_sync(task["id"], "manual")
    ...
    def test_scheduled_occurrences_follow_manual_execution_in_fifo_order(self):
        with self.helper.fixture() as item:
            task = self.helper.create_task(item["app_id"], item["user"])
            ids = [execution_queue._enqueue_task_sync(task["id"], source) for source in ("manual", "schedule", "schedule")]
            for index, execution_id in enumerate(ids):
                self.assertEqual(execution_queue._claim_next_execution(), execution_id)
            ...
            self.assertIsNone(execution_queue._next_pending_execution_id())

    def test_database_still_rejects_simultaneous_running_executions_of_one_task(self):
        with self.helper.fixture() as item:
            task = self.helper.create_task(item["app_id"], item["user"])
            execution_queue._enqueue_task_sync(task["id"], "manual")
            second = execution_queue._enqueue_task_sync(task["id"], "schedule")
            execution_queue._claim_next_execution()
    ...
    def test_restart_preserves_each_pending_scheduled_occurrence(self):
        for state in ("pending", "running"):
            with self.subTest(state=state), self.helper.fixture() as item:
                task = self.helper.create_task(item["app_id"], item["user"])
                manual = execution_queue._enqueue_task_sync(task["id"], "manual")
                planned = [execution_queue._enqueue_task_sync(task["id"], "schedule") for _ in range(2)]
                if state == "running":
                    self.assertEqual(execution_queue._claim_next_execution(), manual)
                database.init_db()
                database.init_db()
    ...
    def test_old_database_index_is_migrated_without_losing_execution_rows(self):
        with self.helper.fixture() as item:
            ...
            database.init_db()
            scheduled = execution_queue._enqueue_task_sync(task["id"], "schedule")
    ...
    def test_cancel_one_occurrence_preserves_other_pending_or_running_work(self):
        for state in ("pending", "running"):
            with self.subTest(state=state), self.helper.fixture() as item:
                task = self.helper.create_task(item["app_id"], item["user"])
                manual = execution_queue._enqueue_task_sync(task["id"], "manual")
                planned = execution_queue._enqueue_task_sync(task["id"], "schedule")
                if state == "running":
                    self.assertEqual(execution_queue._claim_next_execution(), manual)
                with patch.object(security, "write_audit"):
                    executions_api.cancel_execution(planned, object(), item["user"])
    ...
    def test_disabling_cancels_all_pending_occurrences_and_keeps_current_run(self):
        with self.helper.fixture() as item:
            ...
            self.assertEqual(execution_queue._claim_next_execution(), manual)
            with patch.object(security, "write_audit"):
                tasks_api.update_task(task["id"], TaskPatch(enabled=False), object(), item["user"])
            ...
            with self.assertRaises(HTTPException):
                execution_queue._enqueue_task_sync(task["id"], "schedule")
    ...
    def test_worker_failure_keeps_later_schedule_waiting(self):
        with self.helper.fixture() as item:
            ...
            planned = execution_queue._enqueue_task_sync(task["id"], "schedule")
            self.assertEqual(execution_queue._claim_next_execution(), manual)
            execution_queue._mark_execution_worker_failure(manual, "synthetic worker failure")
            self.assertEqual(self.status(task["id"]), "pending")
            self.assertEqual(execution_queue._claim_next_execution(), planned)
