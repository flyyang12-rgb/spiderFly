# DrissionPage：🛰️ 连接浏览器
工具：drissionpage
接入状态：仅知识参考；未接入 SpiderFly AI 执行工具和 collection-v1
主题：🛰️ 连接浏览器；browser_control/connect_browser.md
版本：用户提供的 Markdown 文档 4.1.1.2
核对日期：2026-09-11
适用范围：用户文档原生 API 参考；未进行库运行验证
资料路径：browser_control/connect_browser.md
资料校验：a0cf2ff96f841d68007f48c42ea467977c2fe14fb8f649f686bebe0737f929c5
来源：[官方对应章节（可变网页，不保证与本地版本一致）](https://www.drissionpage.cn/browser_control/connect_browser/)
...
browser = Chromium('wss://****.com/ws?apiKey=EXAMPLE_KEY')
```
...
co = ChromiumOptions().set_address('wss://****.com/ws?apiKey=EXAMPLE_KEY')
browser = Chromium(co)
```
