...
- 当前发布代码仍由单台 Windows 宿主机串行执行任务。工作区已开始局域网 Agent 增量：`api/hosts.py`、`services/host_dispatch.py`、独立 `agent/` 与前端 `features/hosts/`；见 [实际能力](docs/功能地图/宿主机与远程运行.md)、[协议](docs/Agent协议_v1.md)、[需求](docs/多宿主机调度需求.md) 与 [任务拆分](docs/多宿主机调度任务拆分.md)。相邻 `kk-rpa-*` 项目仅作参考。不得把工作区验证描述为正式部署或两机验收。
- 远程运行单独持有机器租约和冻结快照，不送入旧本机队列或旧自动修复。远程失败由独立维护队列生成未运行参考候选，管理员确认后才启用并向原宿主机新建运行。迁移完成前，Agent CLI 与旧主控使用相同机器锁，只在另一台电脑生产运行 Agent；测试专用锁和同机多身份合成验证不能成为生产绕过入口。
...
下述自动修复、试跑与版本启用规则仅描述旧单机链路的迁移前兼容要求。新的多宿主机链路必须遵循已确认的[多宿主机需求](docs/多宿主机调度需求.md)：上传不预试跑、AI 只生成参考候选、管理员确认后才启用和重跑；不得将旧自动启用授权带入远程执行。
