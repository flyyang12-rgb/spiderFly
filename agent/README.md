...
升级只替换固定程序清单（含安装向导、Python 自动安装脚本），保留数据目录、机器身份和 journal，并在 Agent 目录旁保存旧程序 ZIP。当前已在运行的 Agent 不会因主控源码修改而自动更新；升级必须等待本机任务完成、停止 Agent 后人工执行。主控协议仍为 v1，但当前最低 Agent 为 0.3.0；过旧或协议不兼容时，主控拒绝注册/心跳和任务领取，错误会要求人工升级。
