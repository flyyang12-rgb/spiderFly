...
def process(context: TaskContext) -> TaskResult:
    result = run_flow(str(context.input_file), str(context.output_dir / "结果.xlsx"))
    return TaskResult(
        message=f"保留 {result['row_count']} 行，为 {result['processed_row_count']} 行计算平均数；结果已保存。",
        code="EXCEL_AVERAGE_DONE",
    )
